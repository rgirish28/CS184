...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
    ...
    for ( SceneLight *s : scene->lights){
        ...
        for(int i=0;i<num_samples;i++){
            L_in = s->sample_L(hit_p,&wi,&dist,&pdf);
            ...
            r.max_t = dist;
            if(!bvh->intersect(r))
		    L_out+= (L_in*fabs(w_in.z)*isect.bsdf->f(w_in,w_out))/(pdf);
            else
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
    ...
    double termination = 1. - clamp(bsdf.illum() * 10,0.,1.);
    bool terminate = coin_flip(termination);
    ...
    wi.normalize();
    w_in.normalize();
    int recurdepth = r.depth - 1;
    Ray ray = Ray(hit_p + EPS_D * wi, wi,recurdepth);
    Spectrum recursed = trace_ray(ray,isect.bsdf->is_delta());
    return (recursed * bsdf * fabs(dot(wi,hit_n)) )/((1.0-termination)*pdf);
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  Vector2D origin = Vector2D(x,y); // bottom left corner of the pixel
    ...
    if(num_samples == 1){
        x_pos = (double) (x+0.5) / (double) sampleBuffer.w;
        y_pos = (double) (y+0.5) / (double) sampleBuffer.h;
        Ray r = camera->generate_ray(x_pos,y_pos);
        r.depth = max_ray_depth;
        irradiance = trace_ray(r,true);
        i = 1;
    }
        ...
        for(i = 0;i<num_samples;i++){
            ...
            if (i % samplesPerBatch == 0){
                double mu = s1/(double)i;
                double variance = (1/(double) (i-1))*(s2 -((s1*s1)/i));
	    ...
	    Vector2D randomT = gridSampler->get_sample(); 
            x_pos = (double) (x+random.x) / (double) sampleBuffer.w;
            y_pos = (double) (y+random.y) / (double) sampleBuffer.h;
            
            Ray r = camera->generate_ray_for_thin_lens(x_pos,y_pos,randomT.x,randomT.y);
            r.depth = max_ray_depth;
            
            Spectrum value = trace_ray(r,true);
            irradiance += value;
            
            s1 += value.illum();
            s2 += value.illum()*value.illum();
