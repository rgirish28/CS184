...
Color ColorTri::color(Vector2D xy, Vector2D dx, Vector2D dy, SampleParams sp) {
  ...
  //
	...
	Color c =  this->ac * xy.x + this->bc *xy.y + this->cc*(1-xy.x-xy.y);
  return c;
}
...
Color TexTri::color(Vector2D xy, Vector2D dx, Vector2D dy, SampleParams sp) {
  ...
  // Part 7: Fill in the du and dv members of sp as well.
	...
	double u = (double)(xy.x*this->a_uv.x + xy.y*this->b_uv.x + (1-xy.x-xy.y)*this->c_uv.x);
	double v = (double)(xy.x*this->a_uv.y + xy.y*this->b_uv.y + (1-xy.x-xy.y)*this->c_uv.y);

	double u1 = (double)(dx.x*this->a_uv.x + dx.y*this->b_uv.x + (1-dx.x-dx.y)*this->c_uv.x);
	double v1 = (double)(dx.x*this->a_uv.y + dx.y*this->b_uv.y + (1-dx.x-dx.y)*this->c_uv.y);

	double u2 = (double)(dy.x*this->a_uv.x + dy.y*this->b_uv.x + (1-dy.x-dy.y)*this->c_uv.x);
	double v2 = (double)(dy.x*this->a_uv.y + dy.y*this->b_uv.y + (1-dy.x-dy.y)*this->c_uv.y);

	sp.uv = Vector2D(u,v);
	sp.du = Vector2D(u1-u,v1-v);
	sp.dv = Vector2D(u2-u,v2-v);
